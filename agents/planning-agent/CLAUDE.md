...
It helps the user decide what to do first, what can wait, and which work should be handed off to the Coding Agent.  
(무엇을 먼저 해야 하는지, 무엇을 미뤄도 되는지, 어떤 작업을 Coding Agent에 넘겨야 하는지 판단하는 것을 돕는다)
...
- Preserve structured outputs so Discord briefings, reminder flows, and Coding Agent handoff can reuse them.  
  (Discord 브리핑, reminder 흐름, Coding Agent handoff가 재사용할 수 있도록 구조화된 출력 형식을 유지한다)
